...
const docks = new Map();
const __EARTH_MIX = process.env.EARTH_MIX ? process.env.EARTH_MIX.split(',') : null;
const __EARTH_DOCKS = Number(process.env.EARTH_DOCKS) || null;
const { buildHaloPortTrafficLayout } = await import('../src/3d/haloRing/haloPortTraffic.js');
...
for (const node of network.stations) {
  ...
  if (node.id === 'earth' && process.env.EARTH_K7) {
    docks.set(node.id, buildHaloPortTrafficLayout(createHaloRingLayout({}), { id: node.id, x: node.x, y: node.y }));
    continue;
  }
  if (node.id === (process.env.STATION || 'earth') && __EARTH_MIX) {
    docks.set(node.id, buildStationDocks(
      { id: node.id, x: node.x, y: node.y, r: 120, ringWorldRadius: RING_RADIUS[node.id] || 0 },
      { berthMix: __EARTH_MIX, dockCount: __EARTH_DOCKS || 1, berthMultiplier: 1 }
    ));
    continue;
  }
  docks.set(node.id, buildStationDocks(
...
// ---------- pomiar portu Ziemi (K-7) ----------
const __ST = process.env.STATION || 'earth';
const __cls = ['s', 'm', 'l', 'capital', 'mega'];
const __peak = Object.fromEntries(__cls.map(k => [k, 0]));
const __sum = Object.fromEntries(__cls.map(k => [k, 0]));
const __hulls = Object.fromEntries(__cls.map(k => [k, {}]));
const __hist = Object.fromEntries(__cls.map(k => [k, []]));
let __peakQ = 0, __sumQ = 0, __n = 0;
function __sampleEarth(elapsed) {
  const layout = docks.get(__ST);
  if (!layout || elapsed < 600) return;              // pierwsze 10 min = rozruch
  const occ = Object.fromEntries(__cls.map(k => [k, 0]));
  for (const b of layout.berths) {
    if (!b.occupantId) continue;
    occ[b.cls]++;
    const c = registry.byId.get(b.occupantId);
    const hull = c?.unitClass || '?';
    __hulls[b.cls][hull] = (__hulls[b.cls][hull] || 0) + 1;
  }
  for (const k of __cls) { __peak[k] = Math.max(__peak[k], occ[k]); __sum[k] += occ[k]; __hist[k].push(occ[k]); }
  const q = director.portQueue.get(__ST) || 0;
  __peakQ = Math.max(__peakQ, q); __sumQ += q; __n++;
}
...

{
  const layout = docks.get(__ST);
  const tot = {};
  for (const b of layout.berths) tot[b.cls] = (tot[b.cls] || 0) + 1;
  const pct = (arr, p) => { if (!arr.length) return 0; const s = arr.slice().sort((a, b) => a - b); return s[Math.min(s.length - 1, Math.floor(p * s.length))]; };
  console.log('');
  console.log('=== PORT ' + __ST.toUpperCase() + ': zajętość stanowisk (po 10 min rozruchu) ===');
  console.log('  doki: ' + layout.docks.length + ', stanowisk: ' + layout.berths.length + ' ' + JSON.stringify(tot));
  for (const k of __cls) {
    if (!tot[k]) continue;
    const hulls = Object.entries(__hulls[k]).sort((a, b) => b[1] - a[1]).slice(0, 6).map(([h, v]) => h + ' ' + (v / __n).toFixed(2)).join(', ');
    console.log('  ' + k.padEnd(8) + ' średnio ' + (__sum[k] / __n).toFixed(2) + ', p95 ' + pct(__hist[k], 0.95) + ', szczyt ' + __peak[k] + ' / ' + tot[k] + '   kadłuby (śr. zajętych): ' + hulls);
  }
  console.log('  kolejka na redzie: średnio ' + (__sumQ / __n).toFixed(1) + ', szczyt ' + __peakQ);
}
