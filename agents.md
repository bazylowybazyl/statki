...
1. **Core3D (`src/3d/core3d.js`)**
   ...
   - Shadow shafts MNOŻĄ wszystko, co narysowały passy przed nimi (tło, planety, warstwa 0). Emisja w świecie ortho (dysze, pociski, błyski, wiązki, żar i iskry Fx3D, smugi) idzie przez `Core3D.enableOrthoEmissive3D(obj)` — warstwa 8, pass emisji po shaftach razem z tarczami, z głębią kadłubów. Na warstwie 0 w cieniu planety gaśnie razem z bloomem. Tego passa nie trzeba zgłaszać: Core3D sam sprawdza rejestr (widoczność, liczba instancji). Oświetlona materia (dym, kadłuby, odłamki) zostaje na warstwie 0.
   - Cień ma dwie warstwy: tło (mgławica, gwiazdy, planety) dostaje tylko część cienia planety (`SHAFT_VOLUME_STRENGTH` w core3d.js), piksele z głębią passa ortho (kadłuby) — pełny. Ciało rysowane kamerą persp za płaszczyzną gry zgłasza głębokość w `pushShaftDiscWorld(x, y, r, strength, depth)`, żeby cień zaczynał się tam, gdzie ciało widać.
