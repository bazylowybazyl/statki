...
const ATMOSPHERE_VERTEX = `varying vec3 vNormalView; varying vec3 vNormalWorld; varying vec3 vWorldPosition; void main() { vNormalView = normalize(normalMatrix * normal); vNormalWorld = normalize(mat3(modelMatrix) * normal); vec4 worldPos = modelMatrix * vec4(position, 1.0); vWorldPosition = worldPos.xyz; vec4 centerView = modelViewMatrix * vec4(0.0, 0.0, 0.0, 1.0); vec4 viewPos = modelViewMatrix * vec4(position, 1.0); viewPos.z = centerView.z - 20.0; gl_Position = projectionMatrix * viewPos; }`;
const ATMOSPHERE_FRAGMENT = `precision highp float; varying vec3 vNormalView; varying vec3 vNormalWorld; varying vec3 vWorldPosition; uniform vec3 glowColor; uniform vec3 sunsetTint; uniform vec3 sunPosition; uniform float coef; uniform float power; uniform float uSunIntensity; float hash12(vec2 p) { vec3 p3 = fract(vec3(p.xyx) * 0.1031); p3 += dot(p3, p3.yzx + 33.33); return fract((p3.x + p3.y) * p3.z); } void main() { vec3 normalW = normalize(vNormalWorld); vec3 normalV = normalize(vNormalView); float innerToOuter = clamp(abs(normalV.z), 0.0, 1.0); float radialFade = pow(innerToOuter, max(0.35, power * 0.18)); radialFade = smoothstep(0.0, 1.0, radialFade); float rim = radialFade * clamp(coef, 0.0, 2.0); vec3 lightDir = normalize(sunPosition - vWorldPosition); float sunDot = dot(normalW, lightDir); float dayFactor = smoothstep(-0.45, 0.25, sunDot); float sunsetFactor = smoothstep(-0.35, -0.05, sunDot) * (1.0 - smoothstep(-0.05, 0.25, sunDot)); vec3 baseColor = mix(glowColor, sunsetTint * 1.5, sunsetFactor * 0.8); float intensity = rim * (dayFactor + sunsetFactor * 0.3); intensity += (hash12(gl_FragCoord.xy) - 0.5) / 255.0; intensity = clamp(intensity, 0.0, 1.0); gl_FragColor = vec4(baseColor, intensity * clamp(uSunIntensity, 0.2, 1.0)); }`;
...
class DirectPlanet {
    constructor(data) {
        ...
        this.uniforms = {
            ...
            uBrightness: { value: 1.0 }, uAmbient: { value: 0.008 }, uSpecular: { value: 0.22 }, uSunWrap: { value: 0.0 },
            uSunIntensity: { value: 1.12 }, sunsetTint: { value: new THREE.Vector3(1.2, 0.4, 0.1) }
        };
        this.init();
    }
    init() {
        ...
        if (name === 'earth') this.basePlanetBloom = 0.8; else if (name === 'mars') this.basePlanetBloom = 0.4; else if (name === 'jupiter') this.basePlanetBloom = 0.05; else this.basePlanetBloom = 0.2;
